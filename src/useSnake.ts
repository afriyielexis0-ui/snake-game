...
export function useSnake(onGameOver: (score: number, time: number) => void, levelIdx: number) {
  ...
  const step = useCallback(() => {
    ...
    const ateFood = f.x === next.x && f.y === next.y;
    const ateFood2 = f2.x === next.x && f2.y === next.y;
    if (ateFood || ateFood2) {
      ...
      if (ateFood) {
        const newFood = randomPos([...grown, ...obs, food2Ref.current]);
        foodRef.current = newFood;
        setFood(newFood);
      } else {
        const newFood2 = randomPos([...grown, ...obs, foodRef.current]);
        food2Ref.current = newFood2;
        setFood2(newFood2);
      }
      // Refresh obstacles on each food eat
      const newObs = generateObstacles(grown, foodRef.current);
